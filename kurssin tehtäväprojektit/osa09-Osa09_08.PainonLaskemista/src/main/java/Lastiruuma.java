...
public class Lastiruuma {
    ...
    public int yhteispaino() {
        int summa = matkalaukut.stream().map(m -> m.yhteispaino()).reduce(0,(e,luku)->e+luku);
        //int indeksi = 0;
    ...
    public void tulostaTavarat() {
        matkalaukut.stream().forEach(m->m.tulostaTavarat());
        //int indeksi = 0; while (indeksi < this.matkalaukut.size()) {this.matkalaukut.get(indeksi).tulostaTavarat();  indeksi++;
        //}
    }
